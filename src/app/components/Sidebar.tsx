...
export function Sidebar({
  ...
  return (
    <div
      className={`h-screen bg-sidebar text-sidebar-foreground flex flex-col fixed left-0 top-0 transition-all duration-300 z-40 ${collapsed ? "w-20" : "w-64"}`}
    >
      {/* Logo & Collapse Toggle */}
      <div className="p-5 border-b border-sidebar-border flex items-center justify-between">
        <h1
          className={`text-sidebar-primary text-2xl tracking-tight ${collapsed ? "hidden" : ""}`}
        >
          Procast
        </h1>
        <button
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {collapsed && (
                <span className="absolute left-full ml-2 whitespace-nowrap rounded bg-sidebar-accent text-sidebar-primary px-2 py-1 text-xs opacity-0 group-hover:opacity-100 transition-opacity z-[100] pointer-events-none">
                  {item.label}
                </span>
              )}
      ...
      <div className="p-4 border-t border-sidebar-border space-y-3">
        <button
          onClick={onLogout}
          className={`w-full flex items-center gap-3 py-4 rounded-lg text-sidebar-foreground/80 hover:bg-sidebar-accent/50 hover:text-sidebar-accent-foreground transition-all duration-200 transform hover:scale-105 ${collapsed ? "justify-center px-2" : "px-6"}`}
        >
          <LogOut size={20} />
        ...
        <div
          className={`flex items-center gap-3 px-4 py-2 ${collapsed ? "justify-center" : ""}`}
        >
